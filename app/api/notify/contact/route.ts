...
const szolgaltatasLabels: Record<string, string> = {
  teljes: "Teljes körű esküvőszervezés",
  "30nap": "30 nap a nagy napig",
  tanacsadas: "Esküvői tanácsadás",
  egyeb: "Egyéb rendezvények",
};

const honnanLabels: Record<string, string> = {
  instagram: "Instagram",
  facebook: "Facebook",
  google: "Google",
  ismeros: "Ismerős ajánlott",
  szolgaltato: "Szolgáltató ajánlott",
  egyeb: "Egyéb",
};
...
export async function POST(req: NextRequest) {
  ...
  const { nev, email, telefon, datum, letszam, szolgaltatas, honnan, uzenet } = body;

  const rows = [
    ["Név", nev],
    ["E-mail", email],
    ["Telefon", telefon],
    datum ? ["Esküvő dátuma", datum] : null,
    letszam ? ["Vendégek létszáma", letszam] : null,
    szolgaltatas ? ["Szolgáltatás", szolgaltatasLabels[szolgaltatas] ?? szolgaltatas] : null,
    honnan ? ["Honnan hallott rólad?", honnanLabels[honnan] ?? honnan] : null,
    uzenet ? ["Üzenet", uzenet] : null,
    ...
    <div style="background:#f5f3ed;padding:40px 20px;font-family:sans-serif;">
      <div style="max-width:540px;margin:0 auto;background:#fff;border-radius:4px;overflow:hidden;">
        <div style="background:#363025;padding:28px 32px;">
          ...
          <h1 style="margin:8px 0 0;font-family:Georgia,serif;font-size:26px;font-weight:300;color:#fff;">Új megkeresés</h1>
        </div>
