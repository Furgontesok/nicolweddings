      ...
      {showForm && (
        <div className="bg-white border border-[#363025]/10 p-8 mb-10">
          ...
          <div className="flex gap-3 mb-8">
            {[
              { val: "standard", label: "3 fő esküvős szolgáltatás" },
              { val: "egyeb", label: "Egyéb rendezvény" },
            ].map(opt => (
              <button key={opt.val} type="button"
                onClick={() => setForm(f => ({ ...f, service: opt.val }))}
                className={`font-[family-name:var(--font-nunito)] text-[10px] tracking-[0.2em] uppercase px-6 py-2.5 border transition-all duration-200 ${
                  form.service === opt.val
                    ...
                    : "border-[#363025]/25 text-[#363025]/50 hover:border-[#363025]/50"
                }`}>
                {opt.label}
              </button>
            ))}
          ...
          <form onSubmit={editingId ? update : create} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input required className={inputCls} value={form.couple_name}
                ...
                placeholder="Gréti & Domán" />
            </div>
            <div>
              <label className={labelCls}>Esküvő tervezett dátuma</label>
              <input className={inputCls} value={form.wedding_date}
                onChange={e => setForm(f => ({ ...f, wedding_date: e.target.value }))}
                placeholder="2026. szeptember" />
            </div>
            <div>
              <label className={labelCls}>Vendégek létszáma</label>
              <input className={inputCls} value={form.guest_count}
            ...
            {!isEgyeb && (
              <>
                <div className="md:col-span-2">
                  <div className="w-full h-px bg-[#363025]/8 mb-5" />
                  <p className={`${labelCls} mb-4`}>Szolgáltatások árai</p>
                </div>
                <div>
                  <label className={labelCls}>Teljes körű esküvőszervezés — ár</label>
                  <input className={inputCls} value={form.price_teljes}
                    onChange={e => setForm(f => ({ ...f, price_teljes: e.target.value }))}
                    placeholder="pl. 450.000 Ft" />
                </div>
                <div>
                  <label className={labelCls}>30 nap a nagy napig — ár</label>
                  <input className={inputCls} value={form.price_30nap}
                    onChange={e => setForm(f => ({ ...f, price_30nap: e.target.value }))}
                    placeholder="pl. 240.000 Ft" />
                </div>
                <div>
                  <label className={labelCls}>Esküvőszervezési tanácsadás — ár</label>
                  <input className={inputCls} value={form.price_tanacsadas}
                    onChange={e => setForm(f => ({ ...f, price_tanacsadas: e.target.value }))}
                    placeholder="pl. 20.000 Ft/óra" />
                </div>
              </>
            )}
            ...
            {isEgyeb && (
              <div>
                ...
                <input className={inputCls} value={form.price_egyeb}
                  onChange={e => setForm(f => ({ ...f, price_egyeb: e.target.value }))}
                  placeholder="pl. egyedi árazás alapján" />
              </div>
            )}

            <div className="md:col-span-2">
              <label className={labelCls}>Személyes megjegyzés (opcionális)</label>
              <textarea className={`${inputCls} resize-none`} rows={3} value={form.custom_note}
                onChange={e => setForm(f => ({ ...f, custom_note: e.target.value }))}
                placeholder="Különleges megjegyzés, egyedi feltételek..." />
            </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#363025]/8">
          <div className="grid grid-cols-4 px-6 py-3 border-b border-[#363025]/8">
            <span className="font-[family-name:var(--font-nunito)] text-[9px] tracking-[0.25em] uppercase text-[#363025]/35">Pár neve</span>
            <span className="font-[family-name:var(--font-nunito)] text-[9px] tracking-[0.25em] uppercase text-[#363025]/35">Típus</span>
            <span className="font-[family-name:var(--font-nunito)] text-[9px] tracking-[0.25em] uppercase text-[#363025]/35">Létrehozva</span>
            <span className="font-[family-name:var(--font-nunito)] text-[9px] tracking-[0.25em] uppercase text-[#363025]/35">Műveletek</span>
          </div>
          {proposals.map(p => (
            <div key={p.id} className="grid grid-cols-4 px-6 py-4 border-b border-[#363025]/5 hover:bg-[#F5F3ED]/40 transition-colors items-center">
              ...
              <span className="font-[family-name:var(--font-nunito)] text-[11px] text-[#363025]/50">
                {p.service === "egyeb" ? "Egyéb rendezvény" : "3 fő szolgáltatás"}
              </span>
              ...
              <div className="flex items-center gap-4">
                ...
                <a href={`/ajanlat/${p.token}`} target="_blank"
                  className="font-[family-name:var(--font-nunito)] text-[10px] tracking-widest uppercase text-[#363025]/50 hover:text-[#363025] transition-colors">
                  Megtekint
                </a>
